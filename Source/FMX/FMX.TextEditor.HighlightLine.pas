unit FMX.TextEditor.HighlightLine;
...
type
  ...
  published
    property Background: TAlphaColor read FBackground write FBackground default TAlphaColors.Null;
    property Foreground: TAlphaColor read FForeground write FForeground default TAlphaColors.Null;
...
uses
  System.SysUtils, FMX.TextEditor.Language;
...
begin
  ...
  FBackground := TAlphaColors.Null;
  FForeground := TAlphaColors.Null;
  FImported := False;
...
procedure TTextEditorHighlightLineItem.SetOption(const AOption: TTextEditorHighlightLineItemOption; const AEnabled: Boolean);
begin
  if AEnabled then
...
begin
  Result := if FPattern.IsEmpty then STextEditorHighlightLineItemUnnamed else FPattern;
end;
