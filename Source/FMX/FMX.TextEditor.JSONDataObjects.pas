...
unit FMX.TextEditor.JSONDataObjects;
...
uses
  System.Classes, System.SysUtils, System.UITypes, FMX.TextEditor.Consts;
...
type
  ...
  private
    ...
    procedure Init(const AStream: TStream; const AEncoding: TEncoding; const ALines: TStrings);
  ...
  public
    ...
    function ToSingle(const ADefault: Single): Single;
...
uses
  System.AnsiStrings, System.Math, System.RTLConsts, FMX.TextEditor.Language, FMX.TextEditor.Utils;
...
{$IFEND}
  ...
  public
    function GetByteCountEx(AChars: PChar; const ACharCount: Integer): Integer;
    function GetBytesEx(AChars: PChar; const ACharCount: Integer; ABytes: PByte; const AByteCount: Integer): Integer;
    function GetCharCountEx(ABytes: PByte; const AByteCount: Integer): Integer;
    function GetCharsEx(ABytes: PByte; const AByteCount: Integer; AChars: PChar; const ACharCount: Integer): Integer;
  end;
  ...
  private
    FPropName: string;
    procedure Accept(const ATokenKind: TJSONTokenKind);
    procedure AcceptFailed(const ATokenKind: TJSONTokenKind);
  ...
  protected
    ...
    class procedure JSONStrToStr(APos, AEndPos: PChar; AFirstEscapeIndex: Integer; var AText: string; const AReader: TJSONReader); static;
    class procedure JSONUtf8StrToStr(APos, AEndPos: PByte; AFirstEscapeIndex: Integer; var AText: string; const AReader: TJSONReader); static;
    class procedure StringNotClosedError(const Reader: TJSONReader); static;
    function GetCharOffset(const AStartPos: Pointer): NativeInt; virtual; abstract;
  ...
  public
    ...
    destructor Destroy; override;
    procedure Parse(AData: TJSONBaseObject);
  end;
  ...
  private
    ...
    FTextEnd: PByte;
  protected
    function GetCharOffset(const AStartPos: Pointer): NativeInt; override; final;
    function Next: Boolean; override; final;
    procedure LexIdent(AText: PByte);
    procedure LexString(AText: PByte);
  public
    constructor Create(AText: PByte; const ALength: NativeInt);
  end;
  ...
  protected
    function GetCharOffset(const AStartPos: Pointer): NativeInt; override; final;
    function Next: Boolean; override; final;
    procedure LexString(AText: PChar);
    procedure LexIdent(AText: PChar);
  public
    constructor Create(AText: PChar; ALength: Integer);
  end;
...
begin
  ...
  SetLength(S, LOldLen + Len);

  L := Utf8ToUnicode(PWideChar(Pointer(S)) + LOldLen, Len + 1, PAnsiChar(P), Len);

  if L > 0 then
  begin
    if L - 1 <> Len then
      SetLength(S, LOldLen + L - 1);
  end
  else
    SetLength(S, LOldLen);
end;
...
procedure AppendString(var S: string; P: PChar; const ALength: Integer);
var
  LLength: Integer;
begin
  if (P = nil) or (ALength = 0) then
    Exit;

  LLength := S.Length;

  SetLength(S, LLength + ALength);
  Move(P^, PChar(Pointer(S))[LLength], ALength * SizeOf(Char));
end;

class procedure TJSONReader.JSONStrToStr(APos, AEndPos: PChar; AFirstEscapeIndex: Integer; var AText: string;
  const AReader: TJSONReader);
const
  MaxBufferLength = TCharacters.ASCIICharHigh;
var
  LBuffer: array[0..MaxBufferLength] of Char;
  LPChar: PChar;
  LIndex, LLength: Integer;
begin
  Dec(AFirstEscapeIndex);

  if AFirstEscapeIndex > 0 then
  begin
    SetString(AText, APos, AFirstEscapeIndex);
    Inc(APos, AFirstEscapeIndex);
  end
  else
    AText := '';
  ...
  begin
    ...
    while (APos < AEndPos) and (APos^ = '\') do
    begin
      Inc(APos);
      ...
      case APos^ of
        ...
        'u':
          begin
            ...
            if APos + 3 >= AEndPos then
              Break;

            LBuffer[LIndex] := Char(GetHexDigits(APos, 4, AReader));
            Inc(APos, 3);
          end;
      else
        Break;
      ...
      if LIndex > MaxBufferLength then
      begin
        LLength := AText.Length;
        SetLength(AText, LLength + LIndex);
        Move(LBuffer[0], PChar(Pointer(AText))[LLength], LIndex * SizeOf(Char));
        LIndex := 0;
      end;
    end;
    ...
    if APos > LPChar then
      AppendString(AText, LPChar, APos - LPChar);
...
function GetHexDigitsUtf8(AValue: PByte; ACount: Integer; const AReader: TJSONReader): LongWord;
var
  LChar: Byte;
begin
  Result := 0;
  ...
  begin
    ...
    case AValue^ of
      Ord('0')..Ord('9'): Result := (Result shl 4) or LongWord(LChar - Ord('0'));
      Ord('A')..Ord('F'): Result := (Result shl 4) or LongWord(LChar - (Ord('A') - 10));
      Ord('a')..Ord('f'): Result := (Result shl 4) or LongWord(LChar - (Ord('a') - 10));
    else
      Break;
    end;
  ...
  if ACount > 0 then
    raise EJSONParserException.CreateResFmt(@RsInvalidHexNumber, [AValue^], AReader.FLineNum, AReader.GetLineColumn,
      AReader.GetPosition);
end;

class procedure TJSONReader.JSONUtf8StrToStr(APos, AEndPos: PByte; AFirstEscapeIndex: Integer; var AText: string;
  const AReader: TJSONReader);
const
  MaxBufferLength = TCharacters.ASCIICharHigh;
var
...
begin
  ...
  if AFirstEscapeIndex > 0 then
  begin
    SetStringUtf8(AText, APos, AFirstEscapeIndex);
    Inc(APos, AFirstEscapeIndex);
  end
  else
    AText := '';
  ...
  begin
    ...
    while (APos < AEndPos) and (APos^ = Byte(Ord('\'))) do
    begin
      Inc(APos);

      if APos = AEndPos then // broken escaped character
        Break;
      ...
      case APos^ of
        ...
        Ord('\'): LBuffer[LIndex] := '\';
        Ord('/'): LBuffer[LIndex] := '/';
        Ord('b'): LBuffer[LIndex] := #8;
        Ord('f'): LBuffer[LIndex] := #12;
        Ord('n'): LBuffer[LIndex] := #10;
        Ord('r'): LBuffer[LIndex] := #13;
        Ord('t'): LBuffer[LIndex] := #9;
        Ord('u'):
          begin
            Inc(APos);
            ...
            LBuffer[LIndex] := Char(GetHexDigitsUtf8(APos, 4, AReader));
    ...
    if APos > LPos then
      AppendStringUtf8(AText, LPos, APos - LPos);
...
procedure TJSONReader.Parse(AData: TJSONBaseObject);
begin
  if AData is TJSONObject then
  begin
    TJSONObject(AData).Clear;
    Next;
    Accept(jtkLBrace);
    ParseObjectBody(TJSONObject(AData));
    Accept(jtkRBrace);
  end
  else
  if AData is TJSONArray then
  begin
    TJSONArray(AData).Clear;
    Next;
    Accept(jtkLBracket);
    ParseArrayBody(TJSONArray(AData));
    Accept(jtkRBracket);
  end;
end;
...
procedure TJSONReader.AcceptFailed(const ATokenKind: TJSONTokenKind);
var
  LColumn: NativeInt;
  LPosition: NativeInt;
begin
  LColumn := GetLineColumn;
  LPosition := GetPosition;

  if FLook.Kind = jtkEof then
    raise EJSONParserException.CreateResFmt(@RsUnexpectedEndOfFile, [JSONTokenKindToStr[ATokenKind]], FLineNum, LColumn, LPosition);

  raise EJSONParserException.CreateResFmt(@RsUnexpectedToken, [JSONTokenKindToStr[ATokenKind], JSONTokenKindToStr[FLook.Kind]], FLineNum, LColumn, LPosition);
end;

procedure TJSONReader.Accept(const ATokenKind: TJSONTokenKind);
begin
  if FLook.Kind <> ATokenKind then
    AcceptFailed(ATokenKind);
...
begin
  case FDataType of
    ...
    jdtBool:
      Result := if FValue.ValueBoolean then sTrue else sFalse;
  else
...
class procedure TJSONBaseObject.GetStreamBytes(const AStream: TStream; var AEncoding: TEncoding; const AUtf8WithoutBOM: Boolean; var AStreamInfo: TStreamInfo);
var
  LBufStart: Integer;
  LPosition: Integer;
  LSize: NativeInt;
...
begin
  ...
  try
    ...
    begin
      AEncoding := if AUtf8WithoutBOM then TEncoding.UTF8 else TEncoding.Default;
...
function TJSONBaseObject.ToJSON: string;
var
  LJSONOutputWriter: TJSONOutputWriter;
begin
  LJSONOutputWriter.Init(nil, nil, nil);
  try
...
begin
  ...
  begin
    ...
    begin
      for var LIndex := FCapacity to FCount - 1 do
        FItems[LIndex].Clear;
...
begin
  ...
  begin
    ...
    for var LIndex := 0 to ASource.Count - 1 do
...
begin
  ...
  begin
    ...
    for var LIndex := FCount to AValue - 1 do
    begin
...
var
  LItem: PJSONDataValue;
begin
  Result := if FindItem(AName, LItem) then LItem.BoolValue else False;
end;
...
var
  LItem: PJSONDataValue;
begin
  Result :=  if FindItem(AName, LItem) then LItem.Value else '';
end;
...
begin
  ...
  Item := if Result then @FItems[LIndex] else nil;
end;
...
begin
  ...
  for var LSortIndex := LSortCount - 1 downto 0 do
  begin
...
var
  ...
  LLength: Integer;
  LCount: Integer;
  LMiddle, LValue: Integer;
begin
  Result := 0;

  LName := FNames[AIndex];
  LLength := LName.Length;
  LCount := if FFirstUnsortedNameIndex <> -1 then FFirstUnsortedNameIndex - 1 else FCount - 1;
...
begin
  ...
  ReallocMem(FStrings, LLength * SizeOf(FStrings[0]));
  ReallocMem(FBuckets, LLength * SizeOf(FBuckets[0]));
  FillChar(FBuckets[0], LLength * SizeOf(FBuckets[0]), $FF);

  for var LStringIndex := 0 to FCount - 1 do
  begin
    LIndex := (FStrings[LStringIndex].Hash and $7FFFFFFF) mod LLength;
...
procedure TJSONOutputWriter.Init(const AStream: TStream; const AEncoding: TEncoding; const ALines: TStrings);
begin
  FStream := AStream;
  FEncoding := AEncoding;
...
begin
  for var LIndex := 0 to FIndentsLen - 1 do
    FIndents[LIndex] := '';
...
constructor TJSONUTF8Reader.Create(AText: PByte; const ALength: NativeInt);
begin
  inherited Create(AText);

  FText := AText;
  FTextEnd := AText + ALength;
end;

function TJSONUTF8Reader.GetCharOffset(const AStartPos: Pointer): NativeInt;
begin
  Result := FText - PByte(AStartPos);
end;
...
procedure TJSONUTF8Reader.LexString(AText: PByte);
var
  LPEndChar: PByte;
...
begin
  ...
  if LEscapeSequences = nil then
    SetStringUtf8(FLook.Value, AText, LPEndChar - AText)
  else
    TJSONUTF8Reader.JSONUtf8StrToStr(AText, LPEndChar, LEscapeSequences - AText, FLook.Value, Self);
...
procedure TJSONUTF8Reader.LexIdent(AText: PByte);
const
  ...
  case AText^ of
    Ord('A')..Ord('Z'), Ord('a')..Ord('z'), Ord('_'), Ord('$'):
      begin
        ...
        while AText < LPEndChar do
        case AText^ of
          Ord('A')..Ord('Z'), Ord('a')..Ord('z'), Ord('_'), Ord('0')..Ord('9'):
            Inc(AText);
        else
          Break;
        end;

        LLength := AText - LPChar;
        ...
        begin
          ...
          begin
            SetStringUtf8(FLook.Value, LPChar, AText - LPChar);
            FLook.Kind := jtkIdent;
          end;
        end
        else
...
constructor TJSONStringReader.Create(AText: PChar; ALength: Integer);
begin
  inherited Create(AText);
...
function TJSONStringReader.GetCharOffset(const AStartPos: Pointer): NativeInt;
begin
  Result := FText - PChar(AStartPos);
end;
...
procedure TJSONStringReader.LexString(AText: PChar);
var
  LPEndChar: PChar;
...
begin
  ...
  if LEscapeSequences = nil then
    SetString(FLook.Value, AText, LPEndChar - AText)
  else
    TJSONReader.JSONStrToStr(AText, LPEndChar, LEscapeSequences - AText, FLook.Value, Self);
...
procedure TJSONStringReader.LexIdent(AText: PChar);
const
    ...
    'A'..'Z', 'a'..'z', '_', '$':
      begin
        ...
        begin
          ...
          begin
            SetString(FLook.Value, LPChar, AText - LPChar);
            FLook.Kind := jtkIdent;
          end;
        end
        else
...
begin
  ...
  case AValue.FData.DataType of
    ...
    jdtBool:
      Result := if AValue.FData.BoolValue then sTrue else sFalse;
  else
    Result := '';
  end;
end;
...
begin
  ...
  begin
    ...
    Result := if AValue.FData.DataType = jdtObject then TJSONObject(AValue.FData.ObjectValue) else nil;
end;
...
function TJSONDataValueHelper.ToAlphaColor: TAlphaColor;
begin
  var LValue := string(Self).Trim;

  { Theme colors are stored as VCL TColor names and BGR hex values - this is the single place where they
    are converted to TAlphaColor. }

  if LValue.IsEmpty then
    Result := TDefaultColors.SysDefault
  else
  if SameText(LValue, 'clNone') then
    Result := TAlphaColors.Null
  else
  if SameText(LValue, 'clDefault') then
    Result := TDefaultColors.SysDefault
  else
  if SameText(LValue, 'clBlack') then
    Result := TAlphaColors.Black
  else
  if SameText(LValue, 'clWhite') then
    Result := TAlphaColors.White
  else
  if SameText(LValue, 'clRed') then
    Result := TAlphaColors.Red
  else
  if SameText(LValue, 'clGreen') then
    Result := TAlphaColors.Green
  else
  if SameText(LValue, 'clBlue') then
    Result := TAlphaColors.Blue
  else
  if SameText(LValue, 'clYellow') then
    Result := TAlphaColors.Yellow
  else
  if SameText(LValue, 'clAqua') then
    Result := TAlphaColors.Aqua
  else
  if SameText(LValue, 'clFuchsia') then
    Result := TAlphaColors.Fuchsia
  else
  if SameText(LValue, 'clGray') then
    Result := TAlphaColors.Gray
  else
  if SameText(LValue, 'clSilver') then
    Result := TAlphaColors.Silver
  else
  if SameText(LValue, 'clLime') then
    Result := TAlphaColors.Lime
  else
  if SameText(LValue, 'clMaroon') then
    Result := TAlphaColors.Maroon
  else
  if SameText(LValue, 'clNavy') then
    Result := TAlphaColors.Navy
  else
  if SameText(LValue, 'clOlive') then
    Result := TAlphaColors.Olive
  else
  if SameText(LValue, 'clPurple') then
    Result := TAlphaColors.Purple
  else
  if SameText(LValue, 'clTeal') then
    Result := TAlphaColors.Teal
  else
  if SameText(LValue, 'clHighlight') then
    Result := TDefaultColors.SysHighlight
  else
  if SameText(LValue, 'clHighlightText') then
    Result := TDefaultColors.SysHighlightText
  else
  if LValue.StartsWith('$') then
    Result := TextEditorColorToAlphaColor(TColor(StrToUInt64('$FF' + LValue.Substring(1).PadLeft(6, '0'))))
  else
    Result := TextEditorColorToAlphaColor(TColor(StrToUInt64(LValue)));
end;
...
function TJSONDataValueHelper.ToSingle(const ADefault: Single): Single;
var
  LErrorCode: Integer;
begin
  Val(Self, Result, LErrorCode);
...
begin
  ...
  for var LIndex := 1 to Length(Self) do
...
begin
  Result := if FData.Intern <> nil then FData.Intern.DataType else FData.DataType;
end;
...
function TEncodingStrictAccess.GetByteCountEx(AChars: PChar; const ACharCount: Integer): Integer;
begin
  Result := GetByteCount(AChars, ACharCount);
end;

function TEncodingStrictAccess.GetBytesEx(AChars: PChar; const ACharCount: Integer; ABytes: PByte; const AByteCount: Integer): Integer;
begin
  Result := GetBytes(AChars, ACharCount, ABytes, AByteCount);
end;

function TEncodingStrictAccess.GetCharCountEx(ABytes: PByte; const AByteCount: Integer): Integer;
begin
  Result := GetCharCount(ABytes, AByteCount);
end;

function TEncodingStrictAccess.GetCharsEx(ABytes: PByte; const AByteCount: Integer; AChars: PChar; const ACharCount: Integer): Integer;
begin
  Result := GetChars(ABytes, AByteCount, AChars, ACharCount);
end;
...
var
  LDataLength: Integer;
  LString2Length, LString3Length: Integer;
  LLength: Integer;
begin
  LDataLength := FDataLength;
...
procedure TJSONOutputWriter.TJSONStringBuilder.Append3(const AChar1: Char; const AChar2: PChar; const AChar2Length: Integer; const AChar3: Char);
var
  LDataLength: Integer;
  LLength: Integer;
begin
