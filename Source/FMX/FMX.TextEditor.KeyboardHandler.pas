unit FMX.TextEditor.KeyboardHandler;

interface

uses
  System.Classes, System.SysUtils, System.UITypes, FMX.TextEditor.Types, FMX.Types;
...
type
  ...
  public
    ...
    procedure ExecuteKeyDown(ASender: TObject; var Key: Word; var KeyChar: WideChar; Shift: TShiftState);
    procedure ExecuteKeyPress(ASender: TObject; var Key: Char);
    procedure ExecuteKeyUp(ASender: TObject; var Key: Word; var KeyChar: WideChar; Shift: TShiftState);
...
procedure TTextEditorKeyboardHandler.ExecuteKeyDown(ASender: TObject; var Key: Word; var KeyChar: WideChar; Shift: TShiftState);
begin
  if FInKeyDown then
    Exit;
  ...
  try
    ...
    begin
      TKeyEvent(Items[LIndex])(ASender, Key, KeyChar, Shift);
...
procedure TTextEditorKeyboardHandler.ExecuteKeyUp(ASender: TObject; var Key: Word; var KeyChar: WideChar; Shift: TShiftState);
begin
  if FInKeyUp then
    Exit;
...
procedure TTextEditorKeyboardHandler.ExecuteMouseDown(ASender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Single);
begin
  ...
  FInMouseDown := True;
  try
    for var LIndex := FMouseDownChain.Count - 1 downto 0 do
...
procedure TTextEditorKeyboardHandler.ExecuteMouseUp(ASender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Single);
begin
  if FInMouseUp then
    Exit;
  ...
  FInMouseUp := True;
  try
    for var LIndex := FMouseUpChain.Count - 1 downto 0 do
...
procedure TTextEditorKeyboardHandler.ExecuteMouseCursor(ASender: TObject; const ALineCharPos: TTextEditorTextPosition; var ACursor: TCursor);
begin
  ...
  FInMouseCursor := True;
  try
    for var LIndex := FMouseCursorChain.Count - 1 downto 0 do
