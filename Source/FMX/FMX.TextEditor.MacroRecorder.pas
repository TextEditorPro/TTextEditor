unit FMX.TextEditor.MacroRecorder;

interface

uses
  System.Classes, System.SysUtils, System.WideStrUtils, FMX.Controls, FMX.Menus, FMX.TextEditor.KeyCommands, FMX.TextEditor.Language,
...
type
  ...
  public
    ...
    procedure Playback(const AEditor: TControl); virtual; abstract;
  ...
  public
    ...
    procedure Playback(const AEditor: TControl); override;
  ...
  TTextEditorUserCommandEvent = procedure(const ASender: TCustomEditorMacroRecorder; const ACommand: TTextEditorCommand; var AEvent: TTextEditorMacroEvent) of object;
  ...
  strict private
    ...
    function GetEditor: TControl;
    function GetEditorCount: Integer;
    function GetEditors(const AIndex: Integer): TControl;
    function GetEvent(const AIndex: Integer): TTextEditorMacroEvent;
    function GetEventCount: Integer;
    procedure SetEditor(const AValue: TControl);
  protected
    FCurrentEditor: TControl;
    FEditors: TList;
    FEvents: TList;
    ...
    procedure DoAddEditor(const AEditor: TControl);
    procedure DoRemoveEditor(const AEditor: TControl);
    procedure Notification(AComponent: TComponent; AOperation: TOperation); override;
    procedure OnCommand(const ASender: TObject; const AAfterProcessing: Boolean; var AHandled: Boolean; var ACommand: TTextEditorCommand; var AChar: Char; const AData: Pointer);
    procedure SetPlaybackShortCut(const AValue: TShortCut);
    procedure SetRecordShortCut(const AValue: TShortCut);
    procedure StateChanged;
  protected
    procedure HookEditor(const AEditor: TControl; const ACommandID: TTextEditorCommand; const AOldShortCut, ANewShortCut: TShortCut);
    procedure UnHookEditor(const AEditor: TControl; const ACommandID: TTextEditorCommand; const AShortCut: TShortCut);
  ...
  public
    ...
    function AddEditor(const AEditor: TControl): Integer;
    function RemoveEditor(const AEditor: TControl): Integer;
    ...
    procedure Pause;
    procedure PlaybackMacro(const AEditor: TControl; const AUntilEndOfFile: Boolean = False);
    procedure RecordMacro(const AEditor: TControl);
    procedure Resume;
    procedure SaveToFile(const AFilename: string);
    procedure SaveToStream(const ADestination: TStream);
    procedure Stop;
    property EditorCount: Integer read GetEditorCount;
    property Editors[const AIndex: Integer]: TControl read GetEditors;
  ...
  published
    property Editor: TControl read GetEditor write SetEditor;
  end;
...
uses
  System.Types, FMX.Forms, FMX.TextEditor, FMX.TextEditor.Consts, FMX.TextEditor.Utils;
...
procedure TTextEditorDataEvent.Playback(const AEditor: TControl);
begin
...
function TCustomEditorMacroRecorder.AddEditor(const AEditor: TControl): Integer;
begin
...
begin
  ...
  begin
    for var LIndex := FEvents.Count - 1 downto 0 do
    begin
...
begin
  ...
  FRecordShortCut := TextToShortCut('Ctrl+Shift+R');
  FPlaybackShortCut := TextToShortCut('Ctrl+Shift+P');
end;
...
function TCustomEditorMacroRecorder.GetEditors(const AIndex: Integer): TControl;
begin
...
function TCustomEditorMacroRecorder.GetEditor: TControl;
begin
  Result := if Assigned(FEditors) then FEditors[0] else nil;
end;

procedure TCustomEditorMacroRecorder.SetEditor(const AValue: TControl);
var
...
begin
  Result := if Assigned(FEditors) then FEditors.Count else 0;
end;
...
procedure TCustomEditorMacroRecorder.DoAddEditor(const AEditor: TControl);
begin
...
begin
  Result := if Assigned(FEvents) then FEvents.Count else 0;
end;
...
procedure TCustomEditorMacroRecorder.InsertEvent(const AIndex: Integer; const ACommand: TTextEditorCommand; const AChar: Char; const AData: Pointer);
var
...
procedure TCustomEditorMacroRecorder.PlaybackMacro(const AEditor: TControl; const AUntilEndOfFile: Boolean = False);
var
  LEditor: TCustomTextEditor;
...
begin
  ...
  repeat
    ...
    try
      ...
      for var LIndex := 0 to EventCount - 1 do
      begin
...
function TCustomEditorMacroRecorder.RemoveEditor(const AEditor: TControl): Integer;
begin
  if not Assigned(FEditors) then
  begin
    Result := -1;
...
begin
  ...
  for var LIndex := 0 to LCount - 1 do
    Events[LIndex].SaveToStream(ADestination);
end;
...
begin
  ...
  begin
    ...
    begin
      if AValue <> 0 then
      for var LIndex := 0 to FEditors.Count - 1 do
...
procedure TCustomEditorMacroRecorder.HookEditor(const AEditor: TControl; const ACommandID: TTextEditorCommand; const AOldShortCut, ANewShortCut: TShortCut);
var
...
procedure TCustomEditorMacroRecorder.UnHookEditor(const AEditor: TControl; const ACommandID: TTextEditorCommand; const AShortCut: TShortCut);
var
  LEditor: TCustomTextEditor;
  LIndex: Integer;
begin
...
begin
  RepeatCount := StrToIntDef(FMX.TextEditor.Utils.Trim(AString), 1);
end;
...
procedure TTextEditorBasicEvent.Playback(const AEditor: TControl);
var
  LIndex: Integer;
begin
  LIndex := 1;
...
begin
  ...
  Key := if LString.Length >= 1 then LString[1] else ' ';

  Delete(LString, 1, 1);
  RepeatCount := StrToIntDef(FMX.TextEditor.Utils.Trim(LString), 1);
end;
...
procedure TTextEditorCharEvent.Playback(const AEditor: TControl);
var
  LIndex: Integer;
begin
  LIndex := 1;
...
var
  LString: string;
  LDotPosition, LOpenPosition, LClosePosition, X, Y: Integer;
  LValue: string;
begin
  inherited;

  LString := FMX.TextEditor.Utils.Trim(AString);
  ...
  begin
    ...
    LString := FMX.TextEditor.Utils.Trim(LString);
    LClosePosition := Pos(')', LString);
...
begin
  ...
  Position := if Assigned(AData) then TTextEditorTextPosition(AData^) else GetBOFPosition;
end;
...
procedure TTextEditorPositionEvent.Playback(const AEditor: TControl);
begin
...
begin
  ...
  var LSize := LLength * SizeOf(Char);

  GetMem(LPBuffer, LSize);
  try
    FillChar(LPBuffer^, LSize, 0);
    AStream.Read(LPBuffer^, LSize);
    FString := LPBuffer;
  finally
    FreeMem(LPBuffer);
  end;
...
procedure TTextEditorTextEvent.Playback(const AEditor: TControl);
var
  LIndex: Integer;
begin
  LIndex := 1;
  ...
  while LIndex <= RepeatCount do
  begin
    for var LIndex2 := 1 to Value.Length do
...
initialization
  ...
  GroupDescendentsWith(TCustomEditorMacroRecorder, FMX.Controls.TControl);
