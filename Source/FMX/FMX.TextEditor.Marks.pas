unit FMX.TextEditor.Marks;

interface

uses
  System.Classes, System.Contnrs, System.UITypes, FMX.Controls;
...
uses
  System.Types, FMX.TextEditor;

constructor TTextEditorMark.Create(AOwner: TControl);
begin
  inherited Create;
...
constructor TTextEditorMarkList.Create(AOwner: TControl);
begin
  inherited Create;
...
begin
  ...
  if Assigned(LEditor) and Assigned(LEditor.OnBeforeMarkPlaced) then
    LEditor.OnBeforeMarkPlaced(FEditor, AMark);

  if Assigned(AMark) then
    Add(AMark);

  if Assigned(LEditor) and Assigned(LEditor.OnAfterMarkPlaced) then
