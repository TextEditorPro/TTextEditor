unit FMX.TextEditor.Print.HeaderFooter;

interface

uses
  System.Classes, System.SysUtils, System.UITypes, FMX.Graphics, FMX.Types, FMX.TextEditor.Print.Margins, FMX.TextEditor.Types,
...
type
  ...
  published
    ...
    property LineColor: TAlphaColor read FLineColor write FLineColor default TAlphaColors.Black;
    ...
    property ShadedColor: TAlphaColor read FShadedColor write FShadedColor default TAlphaColors.Silver;
  end;
...
uses
  System.Math, System.Types, FMX.TextEditor.Consts;
...
function TTextEditorSectionItem.GetText(const ANumberOfPages, APageNumber: Integer; const ARoman: Boolean; const ATitle, ATime, ADate: string): string;
  ...
  begin
    ...
    LMacro := AnsiUpperCase(Copy(FText, LStart, LRun - LStart + 1));

    if LMacro = '$PAGENUM$' then
    begin
      if ARoman then
    ...
    if LMacro = '$DATETIME$' then
    begin
      DoAppend(ADate + ' ' + ATime);
      Exit;
    end;

    if LMacro = '$TIMEDATE$' then
    begin
...
begin
  ...
  if FMX.TextEditor.Utils.Trim(LString).IsEmpty then
    Exit;
  ...
  begin
    ...
    while LRun <= LLength do
    begin
      if LString[LRun] = '$' then
      begin
        TryAppend(LStart, LRun);
        Inc(LRun);
...
begin
  ...
  begin
    ...
    try
      ...
      PChar(LBuffer)[LBufferSize div SizeOf(Char)] := TControlCharacters.Null;
      FText := PChar(LBuffer);
    finally
      FreeMem(LBuffer);
    end;
    ...
    try
      Read(LBuffer^, LBufferSize);
      PAnsiChar(LBuffer)[LBufferSize div SizeOf(AnsiChar)] := TControlCharacters.Null;
...
begin
  ...
  begin
    ...
    LHeight := -Round(FFont.Size);
    LName := FFont.Family;
    LPitch := TFontPitch.fpDefault;
    LSize := Round(FFont.Size);
    LStyle := FFont.Style;
...
begin
  ...
  FFrameTypes := [ftLine];
  FShadedColor := TAlphaColors.Silver;
  FLineColor := TAlphaColors.Black;
  FItems := TList.Create;
  FDefaultFont := TFont.Create;
  FOldPen := TStrokeBrush.Create(TBrushKind.Solid, TAlphaColors.Black);
  FOldBrush := TBrush.Create(TBrushKind.Solid, TAlphaColors.Black);
...
begin
  ...
  for var LIndex := 0 to FLineInfo.Count - 1 do
...
begin
  ...
  LSectionItem.Font := if Assigned(AFont) then AFont else FDefaultFont;
...
var
  LCurrentLine, LOriginalHeight: Integer;
  LSectionItem: TTextEditorSectionItem;
  LTextHeight: Integer;
begin
  FFrameHeight := -1;
  ...
  begin
    ...
    LTextHeight := Round(TextHeight(ACanvas, 'W'));

    with TTextEditorLineInfo(FLineInfo[LCurrentLine - 1]) do
      LineHeight := Max(LineHeight, LTextHeight);

    FFrameHeight := Max(FFrameHeight, LOriginalHeight + LTextHeight);
  end;
...
begin
  ...
  FOldBrush.Assign(ACanvas.Fill);
  FOldPen.Assign(ACanvas.Stroke);
end;

procedure TTextEditorSection.RestoreFontPenBrush(const ACanvas: TCanvas);
begin
  ACanvas.Font.Assign(FOldFont);
  ACanvas.Fill.Assign(FOldBrush);
  ACanvas.Stroke.Assign(FOldPen);
end;
...
begin
  ...
  ACanvas.Fill.Color := FShadedColor;

  with FMargins do
  begin
    if FrameTypes * [ftBox, ftShaded] <> [] then
    begin
      LRect := if FSectionType = stHeader then RectF(PixelLeft, PixelHeader - FFrameHeight, PixelRight, PixelHeader) else
        RectF(PixelLeft, PixelFooter, PixelRight, PixelFooter + FFrameHeight);

      if ftShaded in FrameTypes then
        ACanvas.FillRect(LRect, 0, 0, [], 1);

      if ftBox in FrameTypes then
        ACanvas.DrawRect(LRect, 0, 0, [], 1);
    end;

    if ftLine in FrameTypes then
    begin
      LLineY := if FSectionType = stHeader then PixelHeader else PixelFooter;

      ACanvas.DrawLine(PointF(PixelLeft, LLineY), PointF(PixelRight, LLineY), 1);
    end;
  end;
end;
...
begin
  ...
  LY := if FSectionType = stHeader then FMargins.PixelHeader - FFrameHeight else FMargins.PixelFooter;
  ...
  begin
    ...
    begin
      ...
      case LAlignment of
        taRightJustify:
          LX := PixelRightTextIndent - Round(LTextWidth);
        taCenter:
          LX := (PixelLeftTextIndent + PixelRightTextIndent - Round(LTextWidth)) shr 1;
      end;
    end;

    { Bottom-aligned within the line box - FMX has no text metrics for true baseline alignment }
...
begin
  ...
  begin
    ...
    Read(LSize, SizeOf(LSize));
    Read(LStyle, SizeOf(LStyle));
    { FMX fonts have no charset, color, height, or pitch - those stream values are read for layout compatibility only }
    FDefaultFont.Family := LName;
...
begin
  ...
  begin
    ...
    { FMX fonts have no charset, color, height, or pitch - placeholders keep the stream layout }
    LCharset := 1;
    LColor := TAlphaColors.Black;
    LHeight := -Round(FDefaultFont.Size);
    LName := FDefaultFont.Family;
    LPitch := TFontPitch.fpDefault;
    LSize := Round(FDefaultFont.Size);
