unit FMX.TextEditor.SyncEdit;

interface

uses
  System.Classes, FMX.TextEditor.Glyph, FMX.TextEditor.Types;
...
uses
  System.UITypes, FMX.TextEditor.Consts, FMX.TextEditor.KeyCommands;
...
begin
  ...
  FInEditor := False;
  FShortCut := TextEditorShortCut(Ord('J'), [ssCtrl, ssShift]);
  FOptions := [seCaseSensitive];
  FSyncItems := TList.Create;
  FActivator := TTextEditorGlyph.Create(igSyncEdit);
end;
...
begin
  for var LIndex := FSyncItems.Count - 1 downto 0 do
