...
begin
  Result := (FCodeFoldingHint.Name <> DEFAULT_FONT) or (FCodeFoldingHint.Size <> TDefaultFontSize.CodeFoldingHint);
end;

function TTextEditorFonts.IsCompletionProposalFontStored: Boolean;
begin
  Result := (FCompletionProposal.Name <> DEFAULT_FONT) or (FCompletionProposal.Size <> TDefaultFontSize.CompletionProposal);
end;

function TTextEditorFonts.IsHintFontStored: Boolean;
begin
  Result := (FHint.Name <> DEFAULT_FONT) or (FHint.Size <> TDefaultFontSize.Hint);
end;

function TTextEditorFonts.IsLineNumbersFontStored: Boolean;
begin
  Result := (FLineNumbers.Name <> DEFAULT_FONT) or (FLineNumbers.Size <> TDefaultFontSize.LineNumbers);
end;

function TTextEditorFonts.IsMinimapFontStored: Boolean;
begin
  Result := (FMinimap.Name <> DEFAULT_FONT) or (FMinimap.Size <> TDefaultFontSize.Minimap);
end;

function TTextEditorFonts.IsRulerFontStored: Boolean;
begin
  Result := (FRuler.Name <> DEFAULT_FONT) or (FRuler.Size <> TDefaultFontSize.Ruler);
end;

function TTextEditorFonts.IsTextFontStored: Boolean;
begin
  Result := (FText.Name <> DEFAULT_FONT) or (FText.Size <> TDefaultFontSize.Text);
end;
...
begin
  ...
  try
    for var LIndex := Low(FFontList) to High(FFontList) do
    if FFontList[LIndex].PixelsPerInch <> AMultiplier then
    begin
      FFontList[LIndex].Height := MulDiv(FBaseHeights[LIndex], AMultiplier, FBasePixelsPerInch[LIndex]);
      FFontList[LIndex].PixelsPerInch := AMultiplier;
    end;
  finally
    FScaling := False;
  end;
...
begin
  FBaseHeights[AIndex] := FFontList[AIndex].Height;
  FBasePixelsPerInch[AIndex] := FFontList[AIndex].PixelsPerInch;
end;
