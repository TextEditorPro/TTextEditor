...
uses
  Winapi.Messages, Winapi.Windows, System.Classes, System.SysUtils, System.UITypes, Vcl.Controls, Vcl.Forms, Vcl.Graphics, TextEditor.Print
{$IFDEF ALPHASKINS}
...
function TTextEditorPrintPreview.GetBackgroundColor: TColor;
begin
  if TStyleManager.IsCustomStyleActive then
  begin
    if Color = TColors.SysAppWorkspace then
...
begin
  ...
  begin
    ...
    Brush.Color := GetBackgroundColor;
    Brush.Style := bsSolid;
    ...
    LPaperRect.Top := if ScaleMode = pscWholePage then FVirtualOffset.Y else FVirtualOffset.Y + FScrollPosition.Y;
...
begin
  ...
  try
    ...
    Inc(LOriginalScreenPoint.Y, if ScaleMode = pscWholePage then FVirtualOffset.Y else FVirtualOffset.Y + FScrollPosition.Y);
  ...
  case FScaleMode of
    pscWholePage:
      begin
        ...
        FPageSize.X := if LWidth < FPageSize.X then LWidth else GetPageHeightFromWidth(FPageSize.X);
      end;
    pscPageWidth:
      begin
        FPageSize.X := ClientWidth - 2 * MARGIN_WIDTH_LEFT_AND_RIGHT;
