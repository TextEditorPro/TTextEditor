...
begin
  ...
  begin
    ...
    begin
      ...
      if ASelect then
      begin
        FPosition.SelectionBegin.Char := LTextPosition.Char;
        FPosition.SelectionBegin.Line := LTextPosition.Line;
        FPosition.SelectionEnd.Char := LChar;
        FPosition.SelectionEnd.Line := LTextPosition.Line;
      end;
    end;
  end;
end;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        for LItemIndex := 0 to FSearch.Items.Count - 1 do
        begin
...
begin
  ...
  begin
    ...
    try
      ...
      AddUndoDelete(LTextPosition, FPosition.SelectionBegin, FPosition.SelectionEnd, LHelper, smNormal);

      LBlockStartPosition := FPosition.SelectionBegin;
      DoSelectedText(S);

      AddUndoInsert(LTextPosition, FPosition.SelectionBegin, FPosition.SelectionEnd, LHelper, smNormal);
    finally
      FUndoList.EndBlock;
    end;
  ...
  begin
    ...
    try
      ...
      AddUndoInsert(LTextPosition, LBlockStartPosition, TextPosition, LHelper, smNormal);
    finally
      if LChangeScrollPastEndOfLine then
        FScroll.SetOption(soPastEndOfLine, False);
    end;
...
begin
  ...
  if toPreviousLineIndent in FTabs.Options then
    if TextEditor.Utils.Trim(FLines[ATextPosition.Line]) = '' then
    begin
  ...
  begin
    if FLines.Columns then
      Result := StringOfChar(TCharacters.Space, LCharCount - (LViewPosition.Column - 1) mod FTabs.Width)
    else
...
begin
  if not Assigned(FLineNumbers.Cache) or not FCodeFolding.Visible or (FLines.Count <= 1) then
    Exit;
...
begin
  ...
  FSelection.Mode := smNormal;
  FUndoList.BeginBlock;
  FUndoList.AddChange(crCaret, TextPosition, FPosition.SelectionBegin, FPosition.SelectionBegin, '',
    FSelection.ActiveMode);
  FPosition.SelectionBegin := ATextBeginPosition;
  FPosition.SelectionEnd := ATextEndPosition;
  SelectedText := AValue;
...
begin
  ...
  begin
    LTextPosition := TextPosition;
    LLineText := FLines.ExpandedStrings[LTextPosition.Line];
    ...
    begin
      ...
      if LPChar^ in [TCharacters.TagOpen, TCharacters.Slash] then
        Break;
    ...
    FMatchingPair.CurrentMatch.OpenToken := TCharacters.TagOpen + LWord;
    FMatchingPair.CurrentMatch.CloseToken := TCharacters.CloseTagOpen + LWord;

    LTempPosition := Pos(FMatchingPair.CurrentMatch.OpenToken, LLineText);

    if LTempPosition > 0 then
      FMatchingPair.CurrentMatch.OpenTokenPos := GetPosition(LTempPosition + 1, LTextPosition.Line); { +1 = < }

    LTempPosition := Pos(FMatchingPair.CurrentMatch.CloseToken, LLineText, LTempPosition);

    if LTempPosition > 0 then
    begin
      FMatchingPair.CurrentMatch.CloseTokenPos := GetPosition(LTempPosition + 2, LTextPosition.Line); { +2 = </ }
      FMatchingPair.Current := trOpenAndCloseTokenFound;
    end;
  end;
end;
